...
export async function setupEventListeners() {
    ...
    // Call setupDOM() to cache the newly created aperture/shading inputs.
    setupDOM();

    // Get the fresh DOM object containing all static and dynamic references
    const dom = getDom();
    ...
    // Safe check for frame toggle, now generated dynamically
    if (dom['frame-toggle']) {
    ...
    // Ensure elements exist (generated by AperturePanelUI)
    if (document.getElementById("sun-ray-date")) {
    ...
    // Sun Ray Trace button is dynamic, so use optional chaining
    ...
    // These IDs are now generated dynamically by AperturePanelUI
    ['n', 's', 'e', 'w'].forEach(dir => {
...
function handleMassingShapeChange() {
    ...
    const selectedShapeEl = document.querySelector('input[name="massing-shape"]:checked');
    if (!selectedShapeEl) return;
    const selectedShape = selectedShapeEl.value;
...
function updateMassingInfo() {
    ...
    const shapeEl = document.querySelector('input[name="massing-shape"]:checked');
    if (!shapeEl) return;
...
async function createMassingBlocks() {
    ...
    const shapeEl = document.querySelector('input[name="massing-shape"]:checked');
    const patternEl = document.querySelector('input[name="massing-pattern"]:checked');

    if (!shapeEl || !patternEl) return;
