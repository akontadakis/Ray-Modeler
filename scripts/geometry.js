...
import { renderer, horizontalClipPlane, verticalClipPlane, sensorTransformControls } from './scene.js';
import { getDom, getAllWindowParams, getAllShadingParams, validateInputs, updateViewpointFromSliders, getWindowParamsForWall, getSensorGridParams } from './ui.js';
import { CSS2DObject } from 'three/addons/renderers/CSS2DRenderer.js';
import { resultsManager } from './resultsManager.js';
import { daylightingSensorsGroup } from './scene.js';
...
export let sensorMeshes = []; // Store references to instanced meshes
export let daylightingSensorMeshes = [];
const shared = {
  lineColor: '#343434',
...
export async function updateScene(changedId = null) {
  ...
  // We must import the selectedWallId state to use it here.
...
/**
 * Creates or moves the AxesHelper to the bottom-left corner of the South wall.
 */
function updateAxesOrigin() {
  const { W, L, rotationY } = readParams();
  let axesHelper = axesObject.getObjectByName('axesHelper');
  if (!axesHelper) {
  ...
  const localCorner = new THREE.Vector3(-W / 2, 0, L / 2);
  const worldCorner = localCorner.clone().applyAxisAngle(new THREE.Vector3(0, 1, 0), rotationY);
    // Set the axes to the world origin
    // axesObject.position.copy(worldCorner);
...
export function createShadingDevices() {
  ...
  for (const [orientation, winParams] of Object.entries(allWindows)) {
    ...
    // This is the key fix: Check for the existence of the specific parameter object.
...
export function updateSensorGridColors(resultsData) {
    if (sensorMeshes.length === 0 || !resultsData || resultsData.length === 0) {
    ...
    sensorMeshes.forEach(mesh => {
        ...
        // No need to create a new attribute, setColorAt handles it.
        for (let i = 0; i < mesh.count; i++) {
...
export function updateDaylightingSensorVisuals() {
    ...
    while (daylightingSensorsGroup.children.length > 0) {
        const obj = daylightingSensorsGroup.children[0];
        obj.traverse(child => disposeMeshLike(child));
        daylightingSensorsGroup.remove(obj);
    }
    daylightingSensorMeshes = [];

    const isEnabled = dom['daylighting-enabled-toggle']?.checked;
    if (!isEnabled) {
        // If the main daylighting system is turned off, ensure the gizmo is also detached and hidden.
    ...
    // Create a container that applies the same corner-origin offset as the room
    const { W, L } = readParams();
