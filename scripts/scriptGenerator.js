...
function _radianceEnvSh(projectData) {
    ...
    return `
# --- Fail fast (generated by Ray Modeler) -----------------------------------
# Without this the script runs past a fatal oconv/dctimestep/python error and
# still prints its own completion banner over empty result files, so a log whose
# last line says "Complete" proves nothing. pipefail is what catches a failure
# inside a pipeline, which is where most Radiance commands sit. The ERR trap
# fires under the same conditions that -e would, but prints the line number
# first; -E makes it reach functions and subshells too. Commands tested inside
# if/while, or joined with && or ||, are exempt, exactly as with -e.
set -o pipefail
set -E
_raymodeler_failed() {
    echo "" >&2
    echo "ERROR: line \$2 exited with status \$1. Aborting." >&2
    echo "       The results below this point were NOT produced." >&2
    exit "\$1"
}
trap '_raymodeler_failed \$? \$LINENO' ERR
# ---------------------------------------------------------------------------
