...
const inputSchema = {
  ...
  recipeParams: {
    ...
    // Generated by ui.js's occupancy panel and registered in
    // project.simulationFiles['occupancy-schedule']; project.js surfaces it in the
    // recipe overrides so this key resolves to the real file.
    'occupancy-schedule': { type: 'file' }
    // 'glare-threshold' / 'glare-autonomy-target' were declared here but the
    // template has no such controls, so they were pinned at defaults.
    // createImagelessGlareScript() already falls back to 0.4 / 95. Removed rather
    // than mapped; re-declare them if the template ever grows the two inputs.
  },
...
function validate(projectData, config) {
  ...
  // Required EPW. Accept the project-level EPW too, matching the seven sibling
  // recipes; checking only simFiles['weather-file'] blocked generation for any
  // project whose weather file was loaded through the normal EPW loader.
  const hasEpw =
